...
function isGenderMatch(n,gender){
    return n.gender.toLowerCase() === gender.toLowerCase();
}
const Male = filter(data, (n)=>{
    return isGenderMatch(n, 'male');
})

console.log(Male.length);


const simplified = clone(data, (n)=>{
    return {value: n.id, label: `${n.first_name} ${n.last_name}`};
});
